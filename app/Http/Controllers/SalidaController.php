...
class SalidaController extends Controller
{
    ...
    public function registrarVenta(Request $request)
    {
        ...
        $proyecto =DB::table('proyectos')->where('id_proyecto',$valores[0]->id_proyecto)->get('id_cliente');
        //get id_cliente of proyecto
        $id_cliente = $proyecto[0]->id_cliente;
        // $cliente = DB::select("select * from cliente where estado=1 and id_cliente=?", [$id_cliente]);
        ...
        /* registrar venta */
        $reVenta = DB::insert("insert into venta(id_cliente,id_usuario,id_proyecto ,fecha,total,descuento,pagoTotal,estado)values(?,?,?,?,?,?,?,1)", [
            $id_cliente,
            $valores[0]->usuario,
            $valores[0]->id_proyecto,
            $fecha,
            $valores[0]->total,
            $valores[0]->descuento,
    ...
    public function edit($id)
    {
        ...
        INNER JOIN usuario ON venta.id_usuario = usuario.id_usuario
         where venta.estado=1 and venta.id_proyecto = ? group by id_venta order by id_venta desc
         ", [$id]);
         $id_proyecto = $id;

         $sqlp = DB::select("select * from proyectos where id_proyecto=? and estado=1", [
            $id,
        ]);
        ...
        $sql2 = DB::select(" SELECT
         ...
         INNER JOIN venta_detalle ON venta_detalle.id_venta = venta.id_venta
         INNER JOIN producto ON venta_detalle.id_producto = producto.id_producto
         where venta.estado=1 and venta.id_proyecto = ?
         order by id_venta asc", [$id]);

        return view("vistas/salida.ventas", compact("sql","id_proyecto","sqlp"))->with("pro", $sql2);
    }
