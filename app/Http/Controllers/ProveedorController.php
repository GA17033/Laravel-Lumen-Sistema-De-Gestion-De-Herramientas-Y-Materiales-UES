...
order by id_proveedor asc");
    ...
    public function store(RegistrarProveedorRequest $request)
    {
        ...
        try {
            $sql = DB::insert("insert into proveedor(nombre,apellido,dni,telefono,direccion,ruc,estado) values(?,?,?,?,?,?,1)", [
                $request->nombre,
                $request->apellido,
                $request->dni,
                $request->telefono,
    ...
    {
        try {
            $sql = DB::update("update proveedor set nombre=?, apellido=?, dni=?, telefono=?, direccion=?, ruc=?
         where id_proveedor=?", [
                $request->nombre,
                $request->apellido,
                $request->dni,
